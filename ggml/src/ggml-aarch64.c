...
// <open-source-office@arm.com> SPDX-License-Identifier: MIT
//
...
static inline __m512i mul_sum_us8_pairs_int32x16(const __m512i ax,
                                                 const __m512i sy) {
...
static inline __m512i mul_sum_i8_pairs_int32x16(const __m512i x,
                                                const __m512i y) {
  const __m512i zero = _mm512_setzero_si512();
...
static inline __m256i mul_sum_us8_pairs_int32x8(const __m256i ax,
                                                const __m256i sy) {
#if defined(__AVXVNNI__) || (defined(__AVX512VNNI__) && defined(__AVX512VL__))
...
static inline __m256i mul_sum_i8_pairs_int32x8(const __m256i x,
                                               const __m256i y) {
#if __AVXVNNIINT8__
  const __m256i zero = _mm256_setzero_si256();
...
/* Computes A^T * b, where A is a matrix and b is a vector
 * n : Number of matrix cols; number of vector rows
 * s : Destination vector
 * bs: Number of matrix rows (unused)
 * vx: Matrix of Q4_0x4 blocks
 * vy: Vector of Q8_0 blocks
 * nr: Number of vector rows (should always be 1 in this case)
 * nc: Number of vector cols; number of matrix cols (because transposed) */
...
/* GEMV. The matrix is in column-major order (mat[x][y] is column x row y)
 * n : Number of matrix cols; length of vector
 * s : Destination vector
 * bs: Number of matrix rows (unused)
 * vx: Matrix of Q4_0 elements
...
void ggml_gemv_q4_0_4x4_q8_0(int n, float *restrict s, size_t bs,
  ...
  /* qk : Quantization level; number of elements per matrix and vector block
   * nb : Number of vector blocks; number of blocks per matrix column
   * ncols_interleaved : Number of blocks per block group; number of columns
   *                     in a block group
   * blocklen : Number of elements per block */
  const int qk = QK8_0;
  const int nb = n / qk;
  const int bg_nb = 4;
  const int b_ne = 4;
  ...
  // assert(v_nb == v_brn)
  /* Is the number of matrix columns a multiple of qk? */
  assert(n % nb == 0);
  /* Can we also make the vector block groups? */
  assert(nc % bg_nb == 0);
...
#if !((defined(_MSC_VER)) && !defined(__clang__)) && defined(__aarch64__) &&   \
    defined(__ARM_NEON) && defined(__ARM_FEATURE_MATMUL_INT8)
  ...
  for (int64_t y = 0; y < nr; y++) {
    ...
    for (int64_t x = 0; x < nc / 8; x++) {
      ...
      for (int64_t b = 0; b < nb; b++) {
        ...
        acc_row = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc),
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
      ...
      for (int l = 0; l < nb; l++) {
        ...
        const float b_scales[8] = {
            GGML_FP16_TO_FP32(b_ptr[l].d[0]), GGML_FP16_TO_FP32(b_ptr[l].d[1]),
 ...
 * s : Destination matrix
 * bs : Number of rows in matrix X
 * vx : Matrix A
 * vy : Matrix B
 * nr : Number of rows in matrix Y
 * nc : Number of cols in matrix Y
 * */
void ggml_gemm_q4_0_4x4_q8_0(int n, float *restrict s, size_t bs,
                             const void *restrict vx, const void *restrict vy,
                             int nr, int nc) {
  /* qk : Quantization level; number of elements in each block
   * nb : Number of blocks
   * */
  const int qk = QK8_0;
  const int nb = n / qk;
       ...
       // defined(__aarch64__) && defined(__ARM_NEON)
  {
    ...
    for (int y = 0; y < nr / 4; y++) {
      const block_q8_0x4 *y_ptr = (const block_q8_0x4 *)vy + (y * nb);
      for (int x = 0; x < nc / ncols_interleaved; x++) {
        const block_q4_0x4 *x_ptr = (const block_q4_0x4 *)vx + (x * nb);
        for (int m = 0; m < 4; m++) {
          for (int j = 0; j < ncols_interleaved; j++)
            sumf[m][j] = 0.0;
        }
          ...
          for (int k = 0; k < (qk / (2 * blocklen)); k++) {
            for (int m = 0; m < 4; m++) {
              for (int j = 0; j < ncols_interleaved; j++) {
                ...
                for (int i = 0; i < blocklen; ++i) {
                  const int v0 =
                      (int8_t)(x_ptr[l].qs[k * ncols_interleaved * blocklen +
                                           j * blocklen + i]
                               << 4);
                  const int v1 =
                      (int8_t)(x_ptr[l].qs[k * ncols_interleaved * blocklen +
                               ...
                               0xF0);
                  sumi +=
                      ((v0 * y_ptr[l].qs[k * 4 * blocklen + m * blocklen + i]) +
                       (v1 * y_ptr[l].qs[k * 4 * blocklen + m * blocklen + i +
                                         qk / 2 * 4])) >>
                      4;
                }
                sumf[m][j] += sumi * GGML_FP16_TO_FP32(x_ptr[l].d[j]) *
                              GGML_FP16_TO_FP32(y_ptr[l].d[m]);
              }
            }
          }
        }
        ...
        for (int k = 0; k < (qk / (2 * blocklen)); k++) {
          for (int m = 0; m < 4; m++) {
            for (int j = 0; j < ncols_interleaved; j++) {
              ...
              sumf[m][j] += sumi * GGML_FP16_TO_FP32(b_ptr[l].d[j]) *
      ...
      for (int64_t x = 0; x < anc / 8; x += 2) {
        ...
        for (int64_t b = 0; b < nb; b++) {
          ...
          const __m512i rhs_raw_mat_014589CD_0 =
              _mm512_inserti32x8(_mm512_castsi256_si512(rhs_raw_mat_0145_0),
                                 rhs_raw_mat_89CD_0, 1);
          const __m512i rhs_raw_mat_2367ABEF_0 =
              _mm512_inserti32x8(_mm512_castsi256_si512(rhs_raw_mat_2367_0),
                                 rhs_raw_mat_ABEF_0, 1);
          const __m512i rhs_raw_mat_014589CD_1 =
              _mm512_inserti32x8(_mm512_castsi256_si512(rhs_raw_mat_0145_1),
                                 rhs_raw_mat_89CD_1, 1);
          const __m512i rhs_raw_mat_2367ABEF_1 =
              _mm512_inserti32x8(_mm512_castsi256_si512(rhs_raw_mat_2367_1),
                                 rhs_raw_mat_ABEF_1, 1);
          ...
          const __m512i rhs_mat_014589CD_0_sp1 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_0,
              136); // B0(0-3) B1(0-3) B0(0-3) B1(0-3) B4(0-3) B5(0-3) B4(0-3)
                    // B5(0-3) B8(0-3) B9(0-3) B8(0-3) B9(0-3) BC(0-3) BD(0-3)
                    // BC(0-3) BD(0-3)
          const __m512i rhs_mat_2367ABEF_0_sp1 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_0,
              136); // B2(0-3) B3(0-3) B2(0-3) B3(0-3) B6(0-3) B7(0-3) B6(0-3)
                    // B7(0-3) BA(0-3) BB(0-3) BA(0-3) BB(0-3) BE(0-3) BF(0-3)
                    // BE(0-3) BF(0-3)

          const __m512i rhs_mat_014589CD_1_sp1 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_1,
              136); // B0(8-11) B1(8-11) B0(8-11) B1(8-11) B4(8-11) B5(8-11)
                    // B4(8-11) B5(8-11) B8(8-11) B9(8-11) B8(8-11) B9(8-11)
                    // BC(8-11) BD(8-11) BC(8-11) BD(8-11)
          const __m512i rhs_mat_2367ABEF_1_sp1 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_1,
              136); // B2(8-11) B3(8-11) B2(8-11) B3(8-11) B6(8-11) B7(8-11)
                    // B6(8-11) B7(8-11) BA(8-11) BB(8-11) BA(8-11) BB(8-11)
                    // BE(8-11) BF(8-11) BE(8-11) BF(8-11)

          const __m512i rhs_mat_014589CD_2_sp1 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_2,
              136); // B0(16-19) B1(16-19) B0(16-19) B1(16-19) B4(16-19)
                    // B5(16-19) B4(16-19) B5(16-19) B8(16-19) B9(16-19)
                    // B8(16-19) B9(16-19) BC(16-19) BD(16-19) BC(16-19)
                    // BD(16-19)
          const __m512i rhs_mat_2367ABEF_2_sp1 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_2,
              136); // B2(16-19) B3(16-19) B2(16-19) B3(16-19) B6(16-19)
                    // B7(16-19) B6(16-19) B7(16-19) BA(16-19) BB(16-19)
                    // BA(16-19) BB(16-19) BE(16-19) BF(16-19) BE(16-19)
                    // BF(16-19)

          const __m512i rhs_mat_014589CD_3_sp1 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_3,
              136); // B0(24-27) B1(24-27) B0(24-27) B1(24-27) B4(24-27)
                    // B5(24-27) B4(24-27) B5(24-27) B8(24-27) B9(24-27)
                    // B8(24-27) B9(24-27) BC(24-27) BD(24-27) BC(24-27)
                    // BD(24-27)
          const __m512i rhs_mat_2367ABEF_3_sp1 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_3,
              136); // B2(24-27) B3(24-27) B2(24-27) B3(24-27) B6(24-27)
                    // B7(24-27) B6(24-27) B7(24-27) BA(24-27) BB(24-27)
                    // BA(24-27) BB(24-27) BE(24-27) BF(24-27) BE(24-27)
                    // BF(24-27)
          ...
          const __m512i rhs_mat_014589CD_0_sp2 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_0,
              221); // B0(4-7) B1(4-7) B0(4-7) B1(4-7) B4(4-7) B5(4-7) B4(4-7)
                    // B5(4-7) B8(4-7) B9(4-7) B8(4-7) B9(4-7) BC(4-7) BD(4-7)
                    // BC(4-7) BD(4-7)
          const __m512i rhs_mat_2367ABEF_0_sp2 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_0,
              221); // B2(4-7) B3(4-7) B2(4-7) B3(4-7) B6(4-7) B7(4-7) B6(4-7)
                    // B7(4-7) BA(4-7) BB(4-7) BA(4-7) BB(4-7) BE(4-7) BF(4-7)
                    // BE(4-7) BF(4-7)

          const __m512i rhs_mat_014589CD_1_sp2 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_1,
              221); // B0(12-15) B1(12-15) B0(12-15) B1(12-15) B4(12-15)
                    // B5(12-15) B4(12-15) B5(12-15) B8(12-15) B9(12-15)
                    // B8(12-15) B9(12-15) BC(12-15) BD(12-15) BC(12-15)
                    // BD(12-15)
          const __m512i rhs_mat_2367ABEF_1_sp2 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_1,
              221); // B2(12-15) B3(12-15) B2(12-15) B3(12-15) B6(12-15)
                    // B7(12-15) B6(12-15) B7(12-15) BA(12-15) BB(12-15)
                    // BA(12-15) BB(12-15) BE(12-15) BF(12-15) BE(12-15)
                    // BF(12-15)

          const __m512i rhs_mat_014589CD_2_sp2 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_2,
              221); // B0(20-23) B1(20-23) B0(20-23) B1(20-23) B4(20-23)
                    // B5(20-23) B4(20-23) B5(20-23) B8(20-23) B9(20-23)
                    // B8(20-23) B9(20-23) BC(20-23) BD(20-23) BC(20-23)
                    // BD(20-23)
          const __m512i rhs_mat_2367ABEF_2_sp2 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_2,
              221); // B2(20-23) B3(20-23) B2(20-23) B3(20-23) B6(20-23)
                    // B7(20-23) B6(20-23) B7(20-23) BA(20-23) BB(20-23)
                    // BA(20-23) BB(20-23) BE(20-23) BF(20-23) BE(20-23)
                    // BF(20-23)

          const __m512i rhs_mat_014589CD_3_sp2 = _mm512_shuffle_epi32(
              rhs_mat_014589CD_3,
              221); // B0(28-31) B1(28-31) B0(28-31) B1(28-31) B4(28-31)
                    // B5(28-31) B4(28-31) B5(28-31) B8(28-31) B9(28-31)
                    // B8(28-31) B9(28-31) BC(28-31) BD(28-31) BC(28-31)
                    // BD(28-31)
          const __m512i rhs_mat_2367ABEF_3_sp2 = _mm512_shuffle_epi32(
              rhs_mat_2367ABEF_3,
              221); // B2(28-31) B3(28-31) B2(28-31) B3(28-31) B6(28-31)
                    // B7(28-31) B6(28-31) B7(28-31) BA(28-31) BB(28-31)
                    // BA(28-31) BB(28-31) BE(28-31) BF(28-31) BE(28-31)
                    // BF(28-31)
          ...
          for (int rp = 0; rp < 4; rp++) {
            ...
            const __m512i lhs_mat_01_0_sp1 = _mm512_shuffle_epi32(
                lhs_mat_01_0,
                160); // A0(0-3) A0(0-3) A1(0-3) A1(0-3) A0(0-3) A0(0-3) A1(0-3)
                      // A1(0-3) A0(0-3) A0(0-3) A1(0-3) A1(0-3) A0(0-3) A0(0-3)
                      // A1(0-3) A1(0-3)
            const __m512i lhs_mat_23_0_sp1 = _mm512_shuffle_epi32(
                lhs_mat_23_0,
                160); // A2(0-3) A2(0-3) A3(0-3) A3(0-3) A2(0-3) A2(0-3) A3(0-3)
                      // A3(0-3) A2(0-3) A2(0-3) A3(0-3) A3(0-3) A2(0-3) A2(0-3)
                      // A3(0-3) A3(0-3)

            const __m512i lhs_mat_01_1_sp1 = _mm512_shuffle_epi32(
                lhs_mat_01_1,
                160); // A0(8-11) A0(8-11) A1(8-11) A1(8-11) A0(8-11) A0(8-11)
                      // A1(8-11) A1(8-11) A0(8-11) A0(8-11) A1(8-11) A1(8-11)
                      // A0(8-11) A0(8-11) A1(8-11) A1(8-11)
            const __m512i lhs_mat_23_1_sp1 = _mm512_shuffle_epi32(
                lhs_mat_23_1,
                160); // A2(8-11) A2(8-11) A3(8-11) A3(8-11) A2(8-11) A2(8-11)
                      // A3(8-11) A3(8-11) A2(8-11) A2(8-11) A3(8-11) A3(8-11)
            ...
            const __m512i lhs_mat_01_0_sp2 = _mm512_shuffle_epi32(
                lhs_mat_01_0,
                245); // A0(4-7) A0(4-7) A1(4-7) A1(4-7) A0(4-7) A0(4-7) A1(4-7)
                      // A1(4-7) A0(4-7) A0(4-7) A1(4-7) A1(4-7) A0(4-7) A0(4-7)
                      // A1(4-7) A1(4-7)
            const __m512i lhs_mat_23_0_sp2 = _mm512_shuffle_epi32(
                lhs_mat_23_0,
                245); // A2(4-7) A2(4-7) A3(4-7) A3(4-7) A2(4-7) A2(4-7) A3(4-7)
                      // A3(4-7) A2(4-7) A2(4-7) A3(4-7) A3(4-7) A2(4-7) A2(4-7)
                      // A3(4-7) A3(4-7)
            ...
            __m512i iacc_mat_00_sp1 = _mm512_add_epi32(
                _mm512_add_epi32(
                    _mm512_add_epi32(
                        mul_sum_i8_pairs_int32x16(lhs_mat_01_3_sp1,
                                                  rhs_mat_014589CD_3_sp1),
                        mul_sum_i8_pairs_int32x16(lhs_mat_01_2_sp1,
                                                  rhs_mat_014589CD_2_sp1)),
                    mul_sum_i8_pairs_int32x16(lhs_mat_01_1_sp1,
                                              rhs_mat_014589CD_1_sp1)),
                mul_sum_i8_pairs_int32x16(lhs_mat_01_0_sp1,
            ...
            __m512i iacc_mat_10_sp1 = _mm512_add_epi32(
                _mm512_add_epi32(
                    _mm512_add_epi32(
                        mul_sum_i8_pairs_int32x16(lhs_mat_23_3_sp1,
                                                  rhs_mat_014589CD_3_sp1),
                        mul_sum_i8_pairs_int32x16(lhs_mat_23_2_sp1,
                                                  rhs_mat_014589CD_2_sp1)),
                    mul_sum_i8_pairs_int32x16(lhs_mat_23_1_sp1,
                                              rhs_mat_014589CD_1_sp1)),
                mul_sum_i8_pairs_int32x16(lhs_mat_23_0_sp1,
                                          rhs_mat_014589CD_0_sp1));
            ...
            __m512i iacc_mat_00_sp2 = _mm512_add_epi32(
                _mm512_add_epi32(
                    _mm512_add_epi32(
                        ...
                        mul_sum_i8_pairs_int32x16(lhs_mat_01_2_sp2,
                                                  rhs_mat_014589CD_2_sp2)),
                    mul_sum_i8_pairs_int32x16(lhs_mat_01_1_sp2,
                                              rhs_mat_014589CD_1_sp2)),
                mul_sum_i8_pairs_int32x16(lhs_mat_01_0_sp2,
            ...
            __m512i iacc_mat_10_sp2 = _mm512_add_epi32(
                _mm512_add_epi32(
                    _mm512_add_epi32(
                        mul_sum_i8_pairs_int32x16(lhs_mat_23_3_sp2,
                                                  rhs_mat_014589CD_3_sp2),
                        mul_sum_i8_pairs_int32x16(lhs_mat_23_2_sp2,
                                                  rhs_mat_014589CD_2_sp2)),
                    mul_sum_i8_pairs_int32x16(lhs_mat_23_1_sp2,
                                              rhs_mat_014589CD_1_sp2)),
                mul_sum_i8_pairs_int32x16(lhs_mat_23_0_sp2,
                                          rhs_mat_014589CD_0_sp2));
            ...
            __m512i iacc_row_0 = _mm512_mask_blend_epi32(
                0xCCCC, iacc_mat_00, _mm512_shuffle_epi32(iacc_mat_01, 78));
            __m512i iacc_row_1 = _mm512_mask_blend_epi32(
                0xCCCC, _mm512_shuffle_epi32(iacc_mat_00, 78), iacc_mat_01);
            __m512i iacc_row_2 = _mm512_mask_blend_epi32(
                0xCCCC, iacc_mat_10, _mm512_shuffle_epi32(iacc_mat_11, 78));
            __m512i iacc_row_3 = _mm512_mask_blend_epi32(
                0xCCCC, _mm512_shuffle_epi32(iacc_mat_10, 78), iacc_mat_11);
        ...
        for (int64_t b = 0; b < nb; b++) {
          ...
          for (int rp = 0; rp < 4; rp++) {
            ...
            __m256i iacc_mat_00_sp1 = _mm256_add_epi32(
                _mm256_add_epi32(
                    _mm256_add_epi32(mul_sum_i8_pairs_int32x8(
                                         lhs_mat_01_3_sp1, rhs_mat_0145_3_sp1),
                                     mul_sum_i8_pairs_int32x8(
                                         lhs_mat_01_2_sp1, rhs_mat_0145_2_sp1)),
                    mul_sum_i8_pairs_int32x8(lhs_mat_01_1_sp1,
                                             rhs_mat_0145_1_sp1)),
                mul_sum_i8_pairs_int32x8(lhs_mat_01_0_sp1, rhs_mat_0145_0_sp1));
            __m256i iacc_mat_01_sp1 = _mm256_add_epi32(
                _mm256_add_epi32(
                    _mm256_add_epi32(mul_sum_i8_pairs_int32x8(
                                         lhs_mat_01_3_sp1, rhs_mat_2367_3_sp1),
                                     mul_sum_i8_pairs_int32x8(
    ...
    for (; y < nr / 4; y++) {
      ...
      for (int64_t x = xstart; x < nc / 8; x++) {
        ...
        for (int64_t b = 0; b < nb; b++) {
          ...
          __m256i iacc_mat_00_sp1 = _mm256_add_epi32(
              _mm256_add_epi32(
                  _mm256_add_epi32(mul_sum_i8_pairs_int32x8(lhs_mat_01_3_sp1,
                                                            rhs_mat_0145_3_sp1),
                                   mul_sum_i8_pairs_int32x8(
                                       lhs_mat_01_2_sp1, rhs_mat_0145_2_sp1)),
                  mul_sum_i8_pairs_int32x8(lhs_mat_01_1_sp1,
          ...
          __m256i iacc_mat_10_sp1 = _mm256_add_epi32(
              _mm256_add_epi32(
                  _mm256_add_epi32(mul_sum_i8_pairs_int32x8(lhs_mat_23_3_sp1,
                                                            rhs_mat_0145_3_sp1),
                                   mul_sum_i8_pairs_int32x8(
                                       lhs_mat_23_2_sp1, rhs_mat_0145_2_sp1)),
                  mul_sum_i8_pairs_int32x8(lhs_mat_23_1_sp1,
          ...
          __m256i iacc_mat_00_sp2 = _mm256_add_epi32(
              _mm256_add_epi32(
                  _mm256_add_epi32(mul_sum_i8_pairs_int32x8(lhs_mat_01_3_sp2,
                                                            rhs_mat_0145_3_sp2),
                                   mul_sum_i8_pairs_int32x8(
                                       lhs_mat_01_2_sp2, rhs_mat_0145_2_sp2)),
                  mul_sum_i8_pairs_int32x8(lhs_mat_01_1_sp2,
          ...
          __m256i iacc_mat_10_sp2 = _mm256_add_epi32(
              _mm256_add_epi32(
                  _mm256_add_epi32(mul_sum_i8_pairs_int32x8(lhs_mat_23_3_sp2,
                                                            rhs_mat_0145_3_sp2),
                                   mul_sum_i8_pairs_int32x8(
                                       lhs_mat_23_2_sp2, rhs_mat_0145_2_sp2)),
                  mul_sum_i8_pairs_int32x8(lhs_mat_23_1_sp2,
    ...
    for (int y = 0; y < nr / 4; y++) {
      ...
      for (int x = 0; x < nc / ncols_interleaved; x++) {
        ...
        for (int l = 0; l < nb; l++) {
          ...
          // vector version needs Zvfhmin extension
          const float a_scales[4] = {GGML_FP16_TO_FP32(a_ptr[l].d[0]),
                                     GGML_FP16_TO_FP32(a_ptr[l].d[1]),
                                     GGML_FP16_TO_FP32(a_ptr[l].d[2]),
                                     GGML_FP16_TO_FP32(a_ptr[l].d[3])};
          const float b_scales[8] = {GGML_FP16_TO_FP32(b_ptr[l].d[0]),
          ...
          __asm__ __volatile__(
              "" ::
                  : "memory"); // prevent gcc from emitting fused vlse64,
                               // violating alignment
          vint16m4_t sumi_l0;
          {
            const vint8m2_t lhs_0_8 = __riscv_vreinterpret_v_i64m2_i8m2(
            ...
            const vint16m4_t sumi_lo_0 =
                __riscv_vwmul_vv_i16m4(rhs_vec_lo_0, lhs_0_8, vl * 2);
            const vint16m4_t sumi_lo_1 = __riscv_vwmacc_vv_i16m4(
                sumi_lo_0, rhs_vec_lo_1, lhs_1_8, vl * 2);
            const vint16m4_t sumi_hi_0 = __riscv_vwmacc_vv_i16m4(
                sumi_lo_1, rhs_vec_hi_0, lhs_2_8, vl * 2);
            const vint16m4_t sumi_hi_m = __riscv_vwmacc_vv_i16m4(
                sumi_hi_0, rhs_vec_hi_1, lhs_3_8, vl * 2);
